...
volatile uint8_t ui8_motor_enabled = 1; // in tsdz2 it was on 1, probably because there is a mode assist used; het set to 0 to force a startup
...
static uint8_t ui8_adc_battery_current_max = ADC_10_BIT_BATTERY_CURRENT_MAX; // In tdsz2 it was 112 = 18A; it is updated by the program based on riding mode parameters
...
void ebike_app_init(void)
{
	...
	// set max motor phase current // used in motor.c to perform some checks // temp = 81*187 = 15147
	uint16_t ui16_temp = ui8_adc_battery_current_max_temp_1 * ADC_10_BIT_MOTOR_PHASE_CURRENT_MAX;//187
	ui8_adc_motor_phase_current_max = (uint8_t)(ui16_temp / ADC_10_BIT_BATTERY_CURRENT_MAX); //112 so 15147/112 = 135
...
{
	// calculate motor ERPS = electrical rotation per sec
    if ((ui16_hall_counter_total >= 0x8000 ) || (ui16_hall_counter_total < 10)) {
	...
	else 
	{
		...
		ui16_motor_speed_erps = ((uint32_t) HALL_COUNTER_FREQ) / ui16_hall_counter_total; // 250000/nrOfTicks; so in sec
	}
...
static void ebike_control_motor(void) // is called every 25ms by ebike_app_controller()
{
	...
	ui8_duty_cycle_ramp_up_inverse_step = PWM_DUTY_CYCLE_RAMP_UP_INVERSE_STEP_DEFAULT;     // 194
    ui8_duty_cycle_ramp_down_inverse_step = PWM_DUTY_CYCLE_RAMP_DOWN_INVERSE_STEP_DEFAULT;  //73
    ui8_adc_battery_current_target = (2+6); // set on 8 for testing but does not really matter because we do not reach this value
											// 1 adc10bits = 0,16A so 1A = 1/0.16=6 : but there is an offset of about 2
	...
	uint8_t ui8_temp_adc_current = (XMC_VADC_GROUP_GetResult(vadc_0_group_0_HW , 8 ) & 0xFFFF) >> 4 ; // current  gr0 ch1 result 8 in queue 0 p2.8 ; from 12 bits to 8 bits
...
#define MOTOR_CHECK_ERPS_THRESHOLD                  	10 // for TSDZ2 with 8 poles :20 ERPS =  1200 erpm = 1200 /8 = 150 rpm
                                                           // for TSDZ8 with 4 poles : 150 rpm => 600 erpm = 10 erps
...
#define BRAKING_FLASH_OFF_COUNTER_MAX     1
    ...
    //------------------------------------------------------------------------------------------------------------------
} // end of ebike_control_lights(void)
*/

/*  ***************** not used in fist test stage **************
...
*/ // end of UART receive and send process


/* ********** not used in the first test **********************	
static void calc_oem_wheel_speed(void)
{ 
	if (ui8_display_ready_flag) {
...
*/ // end of unsude code in first stage 